...
//For broccoli, because it has multiple varieties under cartons but the first one is always the one I want. Starts after var Pkg = (and remember to remove the last };)
//		res.on("end", data => {
//			var rePkg = /Package: (?:(?![&<]).)*/g;//Find the package type
//			var Pkg = body.match(rePkg);//add that to the body
//				if (Pkg[0] === "Package: cartons"){
...
//For celery to get it to check for size and avoid error messages
//res.on("end", data => {
//			var rePkg = /Package: (?:(?![&<]).)*/g;//Find the package type
//			var Pkg = body.match(rePkg);//add that to the body
//			var reSize = /2 dz/g;
