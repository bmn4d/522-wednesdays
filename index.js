...
for(i=0;i<Wednesdays.length;i++){
	var URL = "https://www.marketnews.usda.gov/mnp/fv-report-top-filters?type=movement&locAbr=&locName=&commName=LETTUCE%2C+ROMAINE&startIndex=1&rowDisplayMax=25&portal=fv&navType=byComm&navClass=&termNavClass=&shipNavClass=&movNavClass=&stateID=&volume=&repType=movementDaily&commAbr=LETTR&tranAbr=&environment=&organic=N&repDate="+Wednesdays[i];
	https.get(URL, res => {
		...
		res.on("end", data => {
			var reUnits = /\d{2}\/\d{2}\/20\d{2}<\/span><\/td><td><span>[\w+\s+]+<\/span><\/td><td><span>\d+/g;
